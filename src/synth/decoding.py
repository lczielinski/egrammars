"""Decoding grammars. `build_grammar` returns a grammar whose language is the
programs provably equivalent to the reference (egrammar.py). For branching, the
head grammar offers a whole-box form or the opening `(if (op var number)`;
DynamicRegionRecognizer then rebuilds each arm's grammar over the guard-narrowed
box mid-decode."""
...
def head_grammar(benchmark, box, variables, runs) -> str:
    """A complete no-branch program, or `(FPCore (v) (if (op var number)` stopping
    at the `)`. The shared prefix is one literal so the alternatives don't collide
    in the lexer."""
    vs = " ".join(variables)
    return "\n".join([
...
def arm_grammar(benchmark, box, runs, closes) -> str:
    """A leaf over `box`: leading space, then `closes` trailing `)` (0 for then,
    2 for else)."""
...
class DynamicRegionRecognizer:
    """Grammar recognizer that swaps its own grammar mid-generation: start on the
    head grammar; once it accepts `...(if (op var n)`, rebuild the then-arm's
    e-grammar over the guard's sub-box and continue; likewise then -> else. A
    drop-in for casa's XGrammarTokenRecognizer -- swap points are a deterministic
    function of the emitted prefix, so ASAp's per-prefix trie masks stay valid."""
    ...
    def __init__(self, llm, benchmark, box, saturation):
        ...
        self._arm_cache: dict[tuple, str] = {}
        self.reset()
