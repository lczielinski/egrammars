...
**`skeleton` — constrained skeleton, then fill holes.** The model, constrained to a
skeleton grammar, emits an `if`-tree with `?` arm holes; then each hole's guards narrow
the box, egglog builds a grammar *sound over that sub-box*, and a constrained pass fills
the arm. Sound by construction, but takes multiple generations per program.
...
`fptaylor/<name>-NNN.json`. Flags: `--mode --samples --max-attempts --temperature
